...
from mouse_bluesky.interactive.convenience import ct, test_measure
...
def test_ct_returns_single_detector_count() -> None:
    ...
    counts = ct(seconds=0.0, RE=sim.RE, dets=[sim.detector])
    assert isinstance(counts, float)


def test_test_measure_writes_temp_json() -> None:
    sim = make_peak_bundle()
    result = test_measure(seconds=0.0, RE=sim.RE, dets=[sim.detector], visualize=False)
